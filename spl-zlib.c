...
static int def(void *dest, size_t *destLen, const void *source,
                 size_t sourceLen, int level)
{
	...
	unsigned char *in;
	unsigned char *out;
	int position_in = 0, position_out = 0, size, total_read = 0, total_written = 0;

	IAM_HERE();

	size = sourceLen;
	if (size < 0) {
		pr_err("def invalid sourceLen = %d\n", (int)sourceLen);
		ret = (int)size;
	...
	do {
		...
		len = (total_read + CHUNK < size) ? CHUNK : (size - total_read);
		strm.avail_in = len;
		memcpy(in, source + position_in, len);
		position_in += len; 
		total_read  += len;
		...
		do {
			...
			if (position_out + have > *destLen)
			{
				memcpy(dest + position_out, out, *destLen - position_out);

				pr_err("error: def out of dest buffer\n");
				ret = Z_ERRNO;
				goto fail;
			}
			else
			{
				memcpy(dest + position_out, out, have);
				position_out += have;
				total_written += have;
...
static int inf(void *dest, size_t *destLen, const void *source, size_t sourceLen)
{
	int ret = Z_ERRNO, flush;
	unsigned have;
	z_stream strm;
	unsigned char *in;
	unsigned char *out;
	loff_t position_in = 0, position_out = 0, size, total_read = 0, total_written = 0;

	IAM_HERE();
	size = sourceLen;
	if (size < 0) {
		pr_err("inf invalid sourceLen = %d\n", (int)sourceLen);
		ret = (int)size;
		goto fail_file_reads;
	}
...
z_compress_level(void *dest, size_t *destLen, const void *source,
                 size_t sourceLen, int level)
{
        pr_info("DEBUG WK z_compress_level: sourceLen = %d, destLen = %d, level = %d\n", (int)sourceLen, (int)*destLen, level); 

	return def(dest, destLen, source, sourceLen, level);
}
...
{
        pr_info("DEBUG WK z_uncompress: sourceLen = %d, destLen = %d\n", (int)sourceLen, (int)*destLen); 

	return inf(dest, destLen, source, sourceLen);
}
